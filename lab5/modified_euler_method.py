...
# Розраховуємо систему рівнянь методом модифікованого Ейлера
for step in range(num_steps):
    ...
    x0_half = x0[0] + 0.5 * h * x0_dot
    x1_half = x0[1] + 0.5 * h * x1_dot

    U1_half = U1(t + 0.5 * h)
    x0_dot = (U1_half - x0_half + x1_half * R2) / (C1 * (R1 + R2))
    x1_dot = (((U1_half - x0_half - x1_half * R1) / (R1 + R2)) * R2 - x1_half * R4) / L1
...
plt.legend()
plt.grid()
plt.title("Розв'язок системи рівнянь модфікованим методом Ейлера")
plt.show()
